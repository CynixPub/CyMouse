...
do {
    ...
    Write-Host "==================================================" -ForegroundColor Cyan
    Write-Host "`n[主菜单] 请选择操作功能:" -ForegroundColor Yellow
    Write-Host " 1. 备份 NVS (配置)"
    Write-Host " 2. 备份 LittleFS (文件系统)"
    Write-Host " 3. 恢复 NVS"
    Write-Host " 4. 恢复 LittleFS"
    Write-Host " 5. 完全重刷 (从 0x0 写入全量固件)"
    Write-Host " 6. 固件升级 (仅更新 App 区域)"
    Write-Host " 7. 全片擦除"
    Write-Host " 0. 退出脚本"
    Write-Host "--------------------------------------------------"
    ...
    try {
        switch ($func) {
            "1" { $args = $baseArgs + @("read_flash", $ADDR.nvs, $ADDR.nvs_size, $FILES.nvs); & $esptool $args }
            "2" { $args = $baseArgs + @("read_flash", $ADDR.littlefs, $ADDR.littlefs_size, $FILES.littlefs); & $esptool $args }
            "3" { $args = $baseArgs + @("write_flash", $ADDR.nvs, $FILES.nvs); & $esptool $args }
            "4" { $args = $baseArgs + @("write_flash", $ADDR.littlefs, $FILES.littlefs); & $esptool $args }
            "5" { $args = $baseArgs + @("--after", "hard_reset", "write_flash", "0x0", $FILES.full); & $esptool $args }
            "6" { $args = $baseArgs + @("--after", "hard_reset", "write_flash", $ADDR.app, $FILES.update); & $esptool $args }
            "7" { 
                $confirm = Read-Host "确定要全片擦除吗？(y/n)"
                if ($confirm -eq "y") { $args = $baseArgs + @("erase_flash"); & $esptool $args }
            }
